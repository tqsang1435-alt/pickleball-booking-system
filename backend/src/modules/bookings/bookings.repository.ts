...
import type { CreatedBooking, DailyBooking } from "./bookings.type";
 ...
 */
export async function checkInBookingById(bookingId: number): Promise<void> {
  const pool = await getPool();
...
// ==========================================
// RELEASE EXPIRED (BR-26 + BR-31)
// ==========================================

/**
 * BR-26: Cancel cac booking PendingPayment qua HoldUntil (10 phut).
 * Release slot va schedule ve Available.
 * Returns so luong booking bi cancel.
 */
export async function releaseExpiredHoldings(): Promise<number> {
  const pool = await getPool();
  const transaction = new sql.Transaction(pool);
 ...
 * Returns so luong booking bi mark no-show.
 */
export async function markNoShowExpired(): Promise<number> {
  const pool = await getPool();
 ...
 * Returns so luong booking duoc chuyen sang Completed.
 */
export async function markCompletedExpiredCheckins(): Promise<number> {
  const pool = await getPool();
 ...
 * TODO: Sau khi PlayGroups implement: ALTER TABLE Bookings ADD GroupID INT NULL
 */
export async function createTeamBooking(data: {
  userId: number;
  groupId: number;
  courtId: number;
  slotId: number;
